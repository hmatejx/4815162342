...
class BASICParser:
    ...
    def parse(self, tokenlist, line_number):
        ...
        """
        # Recursively handle multiple statements in the same line
        # This does not work for flow control statements except for STOP
        if Token.COLON in [t.category for t in tokenlist]:
            iter = itertools.groupby(tokenlist, lambda x: x.category == Token.COLON)
            for tlist in [list(g) for k, g in iter if not k]:
                flowsignal = self.parse(tlist, line_number)
                if flowsignal and flowsignal.ftype == FlowSignal.STOP:
                        return flowsignal
    ...
    def status(self):
        """Returns the 'exit status' held in variable RET"""
        if 'RET' in self.__symbol_table:
            return self.__symbol_table['RET']
    ...
    def __simplestmt(self):
        ...
        elif self.__token.category == Token.HOME:
            self.__homestmt()
            return None

        elif self.__token.category == Token.HTAB:
            self.__htabstmt()
            return None

        elif self.__token.category == Token.VTAB:
            self.__vtabstmt()
            return None
        ...
        elif self.__token.category == Token.SND:
            self.__sndstmt()
            return None

        elif self.__token.category == Token.IMG:
            self.__imgstmt()
            return None
    ...
    def __inputstmt(self):
        ...
        self.__print(prompt, end=False, flush=False)
        inputvals = self.__input().split(',', (len(variables)-1))
    ...
    def __term(self):
        ...
        while self.__token.category in [Token.TIMES, Token.DIVIDE, Token.MODULO, Token.EXPONENT]:
            savedcategory = self.__token.category
            self.__advance()
            ...
            elif savedcategory == Token.MODULO:
                self.__operand_stack.append(leftoperand % rightoperand)
    ...
    def __evaluate_function(self, category):
        ...
        # Single parameter functions below
        self.__consume(Token.LEFTPAREN)
    ...
    def __getstmt(self):
        ...
        self.__advance() # Advance past GET token

        if not self.__tokenindex >= len(self.__tokenlist):
            var = self.__token.lexeme

            self.__advance()
            echo = self.__token.category != Token.SEMICOLON
            self.__symbol_table[var] = self.__getc(echo)


    def __homestmt(self):
        """Implements the HOME statement"""
        self.__clrscr()


    def __htabstmt(self):
        """Implements the HTAB statement"""
        self.__advance()
        self.__expr()
        self.__htab(self.__operand_stack.pop())


    def __vtabstmt(self):
        """Implements the VTAB statement"""
        self.__advance()
        self.__expr()
        self.__vtab(self.__operand_stack.pop())


    def __sndstmt(self):
        """Implements the SND statement"""
        self.__advance() # Advance past the IMG token

        self.__expr()
        fname = self.__operand_stack.pop()
        if not isinstance(fname, str):
            raise TypeError("Invalid type supplied to SND in line " +
                            str(self.__line_number))
        if not os.path.exists(fname):
            raise OSError("Could not read file in line " +
                            str(self.__line_number))
        ...
        if self.__token.category == Token.COMMA:
            self.__advance()
            self.__expr()
            vol = self.__operand_stack.pop()

        if not (vol == None or isinstance(vol, float) or isinstance(vol, int)):
            raise TypeError("Invalid type supplied to IMG in line " +
                            str(self.__line_number))

        self.__snd(fname, vol)


    def __imgstmt(self):
        """Implements the IMG statement"""
        self.__advance() # Advance past the IMG token
        ...
        if self.__token.category == Token.COMMA:
            self.__advance()
            self.__expr()
            scale = self.__operand_stack.pop()

            if self.__token.category == Token.COMMA:
                self.__advance()
                self.__expr()
                speed = self.__operand_stack.pop()

        if not (isinstance(xpos, int) and isinstance(ypos, int) and \
                (scale == None or isinstance(scale, float) or isinstance(scale, int)) and \
                (speed == None or isinstance(speed, float) or isinstance(speed, int))):
            raise TypeError("Invalid type supplied to IMG in line " +
                            str(self.__line_number))

        self.__img(fname, xpos, ypos, scale, speed)


    def __pausestmt(self):
        self.__advance()
        self.__expr()
        delay = self.__operand_stack.pop()
        if not isinstance(delay, int):
            raise TypeError("Invalid type supplied to PAUSE in line " +
                            str(self.__line_number))
        self.__pause(delay/1000.)
